%% Adquire and record a CPWC dataset with GE 9LD probe
...
filename='a.mat';
folderdata=[ data_path() '/' datestr(now,'yyyymmdd')];
mkdir(folderdata);            
filedata=['GE9L-D_CPWC_' datestr(now,'HHMMSS') '.uff'];
...
f0=4.21e6;               % central frequency [Hz]
...
no_planes = 31;         % number of acquisitions (ergo plane waves)
...
%% angle sequence
F_number = 3;
maximun_angle=1/2/F_number;    % maximum angle [rad]
if no_planes<2
    angles=0;
else
...
SFormat.transducer = 'GE9L-D';   % 192 element linear array with 0.96 lambda spacing
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'focus', 0.0, ...
                   'Steer', [0.0,0.0], ...
                   'Delay',zeros(1,Trans.numelements)), 1, no_planes);
