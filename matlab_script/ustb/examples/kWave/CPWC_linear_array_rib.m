...
% imaging modalities and how to beamform it with USTB. The k-Wave Toolbox 
% (http://www.k-wave.org) should be in MATLAB's path.
%
...
prb.N=80;                   % number of elements 
prb.pitch=200e-6;           % probe pitch in azimuth [m]
...
scattering_background = false;
if scattering_background
    back_std=0.1/100;
    medium.sound_speed = random('normal',1540,1540*back_std,[domain.N_z_axis, domain.N_x_axis]);   % sound speed [m/s]
    medium.density = random('normal',1020,1020*back_std,[domain.N_z_axis, domain.N_x_axis]);       % density [kg/m3]
end

% include fat layer
fat_std = 0/100;
cn=abs(domain.z-(5e-3 + 0e-3*sin(2*pi*domain.x/5e-3)))<0.5e-3;
...
% include rib
cx=20e-3; cz=10e-3; cr = 5e-3;
cn=sqrt((domain.x-cx).^2+(domain.z-cz).^2)<cr;
medium.sound_speed(cn) = 3600;       % sound speed [m/s]
medium.density(cn) = 1200;           % density [kg/m3]

medium.alpha_coeff(cn) = 6;

cx=20e-3; cz=10e-3; cr = 4.5e-3;
cn=sqrt((domain.x-cx).^2+(domain.z-cz).^2)<cr;
medium.sound_speed(cn) = 3600;       % sound speed [m/s]
medium.density(cn) = 1200;           % density [kg/m3]
...
% include lung
cyst_std = 10/100;
cx=-20e-3; cz=80e-3; cr = 20e-3;
rx=40e-3; ry=60e-3;
cn=sqrt((domain.x-cx).^2/rx.^2+(domain.z-cz).^2/ry.^2)<1;
cn=logical(cn.*(randn(1,length(cn))>0).');
medium.sound_speed(cn) = 543;       % sound speed [m/s]
medium.density(cn) = 10;               % density [kg/m3]

cx=0e-3; cz=80e-3; cr = 20e-3;
rx=32e-3; ry=62e-3;
cn=sqrt((domain.x-cx).^2/rx.^2+(domain.z-cz).^2/ry.^2)<1;
