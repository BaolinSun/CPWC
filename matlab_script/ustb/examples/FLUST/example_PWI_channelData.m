...
s.PSF_params.run.runMode = 'chOnly'; % 'chOnly' just get channel data
...
s.phantom_params.diameter = 0.0002; %0.006;  % Number of flowlines = ceil(diameter/maxLineSpacing)+1
...
%% beamform
BFStart = tic;
[realTabBF, PSFstruct] = doBeamform( realTab, pipe);
s.PSF_params = p;
BFTime = toc(BFStart);

%% VISUALIZE FIRST REALIZATION using the built-in beamformed data object
firstRealization = realTabBF(:,:,:,1,1);
