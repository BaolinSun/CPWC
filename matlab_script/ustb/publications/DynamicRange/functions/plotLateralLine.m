function [f6] = plotLateralLine(sta_image,image,z_depth,x_start,x_stop,colors)
...
for i = 1:length(image.all)
    plot(sta_image.scan.x_axis*1e3,image.all{i}(line_points_1,:)-max(image.all{i}(line_points_1,x_samples_points(1):x_samples_points(2))),'LineWidth',2,'DisplayName',image.tags{i},'Color',colors(i,:))
end
f6 = gca;
end
