...
f0                      = 5.16e+06;      % Transducer center frequency [Hz]
lambda                  = c0/f0           % Wavelength [m]
probe.element_height    = 5e-3;            % Height of element [m]
probe.pitch             = 0.298e-3;        % probe.pitch [m]
probe.element_width     = 0.250e-3;%probe.pitch-kerf;% Width of element [m]
kerf                    = probe.pitch-probe.element_width;% 0.03e-03;        % gap between elements [m]
...
F=7;                                        % number of frames
alpha=0;%linspace(-alpha_max,alpha_max,Na);    % vector of angles [rad]
 
%% Define phantom
% Define some points in a phantom for the simulation
clear point_position point_amplitudes
point_position(1,:,1) = [0 0 abs(probe.x(1)*2)];
point_position(2,:,1) = [0 0 abs(probe.x(1)*2)];
point_amplitudes(:,1) = ones(size(point_position,1),1);

point_position(1,:,2) = [-2e-3 0 abs(probe.x(1)*2)];
point_position(2,:,2) = [2e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,2) = ones(size(point_position,1),1);

point_position(1,:,3) = [-1e-3 0 abs(probe.x(1)*2)];
point_position(2,:,3) = [1e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,3) = ones(size(point_position,1),1);

point_position(1,:,4) = [-0.5e-3 0 abs(probe.x(1)*2)];
point_position(2,:,4) = [0.5e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,4) = ones(size(point_position,1),1);

point_position(1,:,5) = [-0.25e-3 0 abs(probe.x(1)*2)];
point_position(2,:,5) = [0.25e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,5) = ones(size(point_position,1),1);

point_position(1,:,6) = [-0.22e-3 0 abs(probe.x(1)*2)];
point_position(2,:,6) = [0.22e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,6) = ones(size(point_position,1),1);

point_position(1,:,7) = [-0.2e-3 0 abs(probe.x(1)*2)];
point_position(2,:,7) = [0.2e-3 0 abs(probe.x(1)*2)];
point_amplitudes(:,7) = ones(size(point_position,1),1);
%% output data
...
sca=uff.linear_scan('x_axis',linspace(-5e-3,5e-3,256).', 'z_axis', linspace(35e-3,42e-3,256).');
...
channel_data.write([ustb_path(),'/data/FieldII_CPWC_point_scatterers_res_v2.uff'],'channel_data');