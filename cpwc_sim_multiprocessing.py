...
def mat_sim(processid, phantomMatDir, rawDataDir, phantomID, start, end):
    ...
    end_angle = float(end)
    logger.info(f'sub process: {processid}({os.getpid()}) start...')
...
def fork_process(phantomMatDir, rawDataDir, phantomID):
    try:
        ...
        end_time = time.time()
        logger.info(f'consumed time: {end_time - start_time}')
...
if __name__ == '__main__':
    ...
    if os.listdir(phantomImgDir):
        phantomID = findMaxID(phantomImgDir)
    else:
        phantomID = 0
    
    logger.info(f'Start simulate phantom from ID-{phantomID}')
    ...
    try:
        ...
        while True:
            ...
            logger.info('==>generate phantom matrix...')
            mat_generate_phantom(phantomImgDir, phantomMatDir, phantomID, N)

            logger.info('==>simulate cpwc with multiprocessing...')
